...
view1.onclick = function(){
  ...
  if(fpode.length)
    result.innerHTML = `<span style="color: blue;">${fpode.join(', ')}<span><br>`

  if(mpode.length)
    result.innerHTML += `<span style="color: deeppink;">${mpode.join(', ')}<span>`
}
...
view2.onclick = function(){
  ...
  if(fdeve.length===0 && mdeve.length===0)
    result.innerText = 'Não há pessoas cadastradas que devem obrigatoriamente votar'

    if(fdeve.length)
    result.innerHTML = `<span style="color: blue;">${fdeve.join(', ')}<span><br>`

  if(mdeve.length)
    result.innerHTML += `<span style="color: deeppink;">${mdeve.join(', ')}<span>`
}